...
class CBAMResNet_ae(nn.Module):
    def __init__(self, device, ae, num_layers, feature_dim=512, drop_ratio=0.4, mode='ir',filter_list=filter_list):
        super(CBAMResNet_ae, self).__init__()
        assert num_layers in [50, 100, 152], 'num_layers should be 50, 100 or 152'
        assert mode in ['ir', 'ir_se', 'ir_cam', 'ir_sam', 'ir_cbam'], 'mode should be ir, ir_se, ir_cam, ir_sam or ir_cbam'
        layers = get_layers(num_layers)
        if mode == 'ir':
        ...
        self.h2normal = nn.Linear(6400, 512)
        self.x2gate  = nn.Linear(512, 1)
        self.concat2final = nn.Linear(512*2, 512)
        self.bn1d = nn.BatchNorm1d(feature_dim)
    ...
    def forward(self, x):
        ...
        h = self.relu(self.h2normal(h)) # b 512
        
        gate = self.sigmoid(self.x2gate(x)) * 512
        gate = gate.int().squeeze()# b
        #rint(gate.shape)
        double = torch.cat((h,x), dim = -1) # b 1024
        mask = torch.ones_like(double).to(self.device) - length_to_mask(gate, max_len=1024, dtype=torch.float)
#         for i in range(mask.shape[0]):
#             for j in range(gate[i]):
#                 mask[i,j] = 0
#         print(torch.sum(mask[0]))
#         print(torch.sum(mask[1]))
#         print(torch.sum(mask[2]))
#         print(mask)
#         print(sum(mask[0]))
#         print(sum(mask[1]))
        double = double * mask
        #print(double.shape)
        final = self.bn1d(self.concat2final(double))

        

        return final
    
def length_to_mask(length, max_len=512, dtype=torch.int):
    """length: B.
    return B x max_len.
